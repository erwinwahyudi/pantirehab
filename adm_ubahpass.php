<div class="span7">
	<div class="box gradient">
		...
		<div class="content">
			...
			?>
				<form class="form-horizontal row-fluid" action="?menu=adm_ubahpass" method="post">
	...
	if (isset($_POST['action']) == 'update')
	{
		...
		$passkon = md5($_POST['passkon']);
		 ...
		 $row = mysql_fetch_array($query);
		 $passadmin = $row['pass'];
		 if($passlama == $passadmin && $passbaru==$passkon) {
		 	$update = mysql_query("UPDATE admin SET id_admin='$id_admin', pass='$passbaru' WHERE id_admin='$id_admin' ");
		 	if($update){
		 		?> <script language="javascript">alert("Password baru tersimpan");
					document.location="?menu=adm_ubahpass";</script>
	   			<?php
		 	} else {
